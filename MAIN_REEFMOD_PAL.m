...
% For testing
% h2=1.0; % heritability
% ssp=1; % greenhouse gas emissions scenario (Shared Socioeconoic Pathway)
% gcm=11; % Global Circulation Model
% ss = 0; % Self-seeding adjustment factor
% BH_a = 0; % Beverton-Holt function density dependent settlement adjustment factor
% BH_b = 0; % Beverton-Holt function density dependent settlement adjustment factor
...
% (R)= h2 * (Zs - Zp). (1) Zp is from the ancestral population.
...
if NB_TIME_STEPS > 30   
    if OPTIONS.doing_DHWadaptation == 0
        ...
        OPTIONS.OutputFileName = [SaveDir OutputName sprintf('%1.1f',h2) ET_addon '_' Trunc '_' char(OPTIONS.GCM) '_' char(OPTIONS.SSP) DHWrandom '.mat'];
    end
else
