...
% clearvars -except mo
% 
% modeloptions = ["classic", "DHWbleaching", "DHWadaptation"];
% DHWbleaching = [0 1 1];
% DHWadaptation = [0 0 1];
...
% Here you can set the file name. For 6 species I have been using Rx_PAL.
% For 1 species models I have been using Ax_PAL. x refers to (0 classic
% reefmod, 1 DHWadaptation off, or 2 DHWadaptation on.	Here you can set
% the file name. For 6 species I have been using Rx_PAL. For 1 species
% models I have been using Ax_PAL. x refers to (0 classic reefmod, 1
% DHWadaptation off, or 2 DHWadaptation on). pub = Nsimulations
% OutputName = char('RM_',strcat(modeloptions(mo),'_FORECAST_PAL')); 
OutputName = 'A2_FORECAST_PAL_h2'; 
% OutputName = 'A0_FORECAST_PAL'; 

options = [0 1 0 0 0]; % see options below
...
OPTIONS.doing_DHWbleaching = 1; %DHWbleaching(mo); %0 ;
OPTIONS.doing_DHWadaptation = 1; %DHWadaptation(mo); %0 ;
OPTIONS.doing_DHWadaptation_covZw = 0; % Prices Equation: Selection Differential based on covariance between the trait and fitness After discussion with Alastair Wilson
if OPTIONS.doing_DHWadaptation_covZw; PricesEq='_PE'; else; PricesEq=''; end
OPTIONS.generation_length = g; % working with 1 so far to compare to previous method
OPTIONS.doing_DHWadaptation_VA = 0; % doing additive genetic inheritance
% For the selection differential (S) in the breeders equation, Zs is the
% selected parents and Zp is the population mean. 
% Response to selection (R)= h2 * (Zs - Zp). Is Zp from (1) the ancestral
% population, (2) the population at parents birth for the mean generation
% time that we guess is 10 years, or a weighted average Zp, or (3) based on
% the specific Zp at parental birth of all different selected parental age
% groups (see JW James 1979 on selction with overlapping generations).
OPTIONS.doing_DHWadaptation_Zp = 1; % 1, 2 or 3, see above, or 4 (population mean Z of gravid colonies before selection)
OPTIONS.doing_coral_age = 0; % (0) is off, (1) on. Needs to be on for Zp method = 3 only
...
if NB_TIME_STEPS > 30   
    if OPTIONS.doing_DHWadaptation == 0
        ...
        OPTIONS.OutputFileName = [SaveDir OutputName sprintf('%1.1f',h2) PricesEq ET_addon '_' Trunc '_' char(OPTIONS.GCM) '_' char(OPTIONS.SSP) DHWrandom '.mat'];
    end
else
