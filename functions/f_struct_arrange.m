...
for s = 1:META.nb_coral_types
    ...
    if META.doing_DHWbleaching == 1 
        temp_heat_tolerance = coral(s).heat_tolerance(nIdx);
        temp_heat_tolerance(:,id_sum==0)=[] ;
        coral(s).heat_tolerance = sparse(temp_heat_tolerance) ;
    end
    
    if META.doing_coral_age == 1 
        temp_coral_age = coral(s).coral_age(nIdx);
        temp_coral_age(:,id_sum==0)=[] ;
        coral(s).coral_age = sparse(temp_coral_age) ;
    end
