...
function [coral, algal, total_coral_loss, total_mortality, cov_z_w, Zp_mu] = f_bleaching_Liam(coral, algal, bleaching_whole_mortality, DHW, ...
    CORAL, doing_3D, nb_coral_types, doing_clades, doing_DHWbleaching, doing_DHWadaptation_covZw, doing_coral_age, bleaching_whole_offset, bleaching_partial_offset)
% Zp_mu = the parental mean before selection
% cov_z_w = the covariance between the trait and relative fitness (before selection)
...
cov_z_w = zeros(1,nb_coral_types);
...
for s = 1:nb_coral_types
    ...
    id1 = ones(size(coral(s).cover_cm2)) ; % assigns 1 to every colony
    % id1(coral_cm2 < CORAL.adol_size) = 0 ; % Only keep adults + adol (note this also excludes negative/dead colonies for speed)
    id1(coral(s).cover_cm2 < CORAL.size_threshold_wcm) = 0 ; % Only keep adults + adol (note this also excludes negative/dead colonies for speed)
    ...
    %                         1 + exp(B0 + B1 * DHW)
    %
    LinearEq = exp(coral(s).heat_tolerance + CORAL.DHWbleaching_mortality_Slope * DHW);
    bleaching_mortality_prob =  (LinearEq ./ (1+LinearEq));
    bleaching_mortality_prob(coral(s).cover_cm2 <= 0) = 0; % remove already dead ones
    ...
    % --------------------------------------------------------------------------
    % INSERT FECUNDITY-WEIGHTED RESPONSE TO SELECTION BEFORE APPLYING MORTALITY
    % --------------------------------------------------------------------------
    ...
    % If there are any gravid colonies
    if full(sum(coral(s).cover_cm2 > CORAL.fecund_min_size(s),'all')) == 0
        cov_z_w(s) = 0;
        Zp_mu(s) = NaN;
    else
        
        
        z = full(coral(s).heat_tolerance(coral(s).cover_cm2 > CORAL.fecund_min_size(s))); % same as HT_adult_g
        w = 1 - full(prob_whole_mortality(coral(s).cover_cm2 > CORAL.fecund_min_size(s)));
        ...
        %     cov_z_w(s) = sum((z-mean(z)).*(rw-mean(rw)))./length(z);
        %     R_response_to_selection(s) = CORAL.DHWbleaching_mortality_h2 * sum((z-mean(z)).*(rw-mean(rw)))./length(z);
        
        
        % compute response to selection based on fecund adult corals.
        % DO NOT NEED to weight by colony fecundity as it does not change the
        % covariance (simply repeated numbers in the two vectors z and w), BUT
        % DO NEED IT FOR THE Zp_mu.
        ...
        adult_coral_sizes = full(coral(s).cover_cm2(coral(s).cover_cm2 > CORAL.fecund_min_size(s)));
        all_egg_volumes = exp(CORAL.fecund_a(s) + CORAL.fecund_b(s)*log(1.*adult_coral_sizes)) ; % mm3 of eggs produced by each colony
        %         fecundity = floor(sum(sum(all_egg_volumes))/0.1) ; %0.1 mm3 is the average volume of an egg
        ...
        % fecundity weighted (fw) lists of trait values and fitness
        z_fw = repelem(z, rep_fec); % then repeat the HTs of each colony by the fecundity
        Zp_mu(s) = mean(z_fw);
        w_fw = repelem(w, rep_fec); % then repeat the HTs of each colony by the fecundity
        
        rw_fw = w_fw / mean(w_fw);
        %     scatter(z,rw);
        %     cv = cov(z,rw);
        %     cv = cv(2,1);
        %     cv = sum((z-mean(z)).*(rw-mean(rw)))./length(z);
        %     R = CORAL.DHWbleaching_mortality_h2 * cv
        cov_z_w(s) = sum((z_fw-mean(z_fw)).*(rw_fw-mean(rw_fw)))./length(z_fw);
        %     R_response_to_selection(s) = CORAL.DHWbleaching_mortality_h2 * sum((z-mean(z)).*(rw-mean(rw)))./length(z);
    end
    ...
    coral_loss = coral(s).cover_cm2.*id_dead ;
    
    algal_cm2(:,1) = algal_cm2(:,1) + sum(coral_loss,2) ;
    % coral_cm2(id_dead==1) = - coral_cm2(id_dead==1);  % now dead (negatives)
    coral(s).cover_cm2(id_dead==1) = 0;
    % ht = heat_tolerance(heat_tolerance~=0); mean(ht,'all')
    coral(s).heat_tolerance(id_dead==1) = 0;
    % ht = heat_tolerance(heat_tolerance~=0); mean(ht,'all')
    if doing_coral_age == 1
        coral(s).coral_age(id_dead==1) = 0; % update so dead corals have age zero
    end
    ...
    bleach_extent = floor(CORAL.bleaching_partial_extent(s).* coral(s).cover_cm2 .* id_part) ;
    algal_cm2(:,1) = algal_cm2(:,1) + sum(bleach_extent, 2) ;
    coral(s).cover_cm2 = coral(s).cover_cm2 - bleach_extent ;
    ...
    total_coral_loss(s) = sum(sum(coral_loss))+sum(sum(bleach_extent));
...
% col_stop = 0;
% 
% %% Added heat tolerance & Response to selection using Price Eq (July 2023)
