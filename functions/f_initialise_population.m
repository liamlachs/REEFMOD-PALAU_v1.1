...
for s = 1:META.nb_coral_types
    ...
    if META.doing_coral_age == 1
        % Added by Liam. Need to know coral age
        % To compute overlapping generation selection differential on heat
        % tolerance we must know each corals age (to closest 6 months)
        % Estimate age of each coral. Note that there is a maximum juvenile
        % diameter, and below this all juvenile corals have a growth rate of
        % 0.5cm radius/6months (1cm/yr). So based on growth rates, we can
        % estimate age for the contribution of radius > and < juvenile max size
        temp_radius = sqrt(coral(s).cover_cm2/pi);
        max_rad_juv_contribution = sqrt(CORAL.juv_max_size/pi);
        
        temp_rad_adult_contribution = temp_radius - max_rad_juv_contribution;
        temp_rad_adult_contribution(temp_radius <= max_rad_juv_contribution)=0;
        temp_age_adult_contribution = temp_rad_adult_contribution / (CORAL.growth_rate(s)*2); % in years for a 6month growth rate
        
        temp_rad_juv_contribution = temp_radius;
        temp_rad_juv_contribution(temp_radius > max_rad_juv_contribution) = max_rad_juv_contribution;
        temp_age_juv_contribution = temp_rad_juv_contribution / (CORAL.juvenile_growth_rate*2); % in years for a 6month growth rate
        temp_age = temp_age_adult_contribution + temp_age_juv_contribution;
        temp_age(temp_radius == 0)=0;
        coral(s).coral_age = round(temp_age,0); % in years, safe to do this, we won't lose any tiny coral as tiny ones can be minimum 2cm2 which translates to 0.79 years-old radius which rounds up to 1
    else
        coral(s).coral_age = 0;
    end
    ...
    if META.doing_DHWbleaching == 1
        % Assign a heat tolerance intercept for the bleaching mortality vs DHW sigmoidal curve
        coral(s).heat_tolerance = spones(coral(s).cover_cm2); %default is 1 (thermally sensitive, clade C)
        if META.doing_DHWbleaching_TaxaSensivities == 0
            HT_distribution = normrnd(CORAL.DHWbleaching_mortality_Intercept_av, ...
                CORAL.DHWbleaching_mortality_Intercept_sd, ...
                size(coral(s).heat_tolerance)) ; % randomly assign heat tolerance as mean and SD of sigmoidal curve intercepts that correspond to Lachs et al 2022
        else
            HT_distribution = normrnd(CORAL.DHWbleaching_mortality_Intercept_av*(CORAL.DHWsensitivity_bleaching(s)), ...
                CORAL.DHWbleaching_mortality_Intercept_sd, ...
                size(coral(s).heat_tolerance)) ; % randomly assign heat tolerance as mean and SD of sigmoidal curve intercepts that correspond to Lachs et al 2022
        end    
    if META.doing_Truncated == 1
         HT_distribution(HT_distribution  <= CORAL.DHWbleaching_mortality_Intercept_super_tolerant(s)) = CORAL.DHWbleaching_mortality_Intercept_super_tolerant(s);
    end
    coral(s).heat_tolerance = coral(s).heat_tolerance .* HT_distribution; % multiply by 1 or zero to remove non-colonies
    else
        coral(s).heat_tolerance = 0;
    end
