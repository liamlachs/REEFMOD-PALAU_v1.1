...
% assume 30% coral cover across all sites - see Marine Gouezos paper in Proc B
% relative cover (rcs) of coral groups 
% reefmod (1-Stagorn Acropora) (2-Plating Acropora) (3-Corymbose Acropora)
% (4-Corymbose non-Acropora) (5-Submassive & Encrusting) (6-Large Massive)
% relative covers based on size of bars in 2010 from Gouezo each column is
% one habitat type/dept outer reef west (10m 3m) outer reef east (3m 10m)
% groups are (reefmod category in brackets) Acrop (1,2,3), Monti (5), 
% Porit (6), Merulin (5), Pocillo (4), Agaraciid (5), 
% Other (?, Leave out as cannot fit into reefmod classification)
% units in cm from graph
rcs = [ 1.0 1.0 0.5 0.6; ...
        1.0 1.0 0.7 0.5; ...
        1.8 0.4 0.6 1.0; ...
        1.0 0.5 1.0 0.8; ...
        ...
        0.2 0.01 0.01 0.1; ...
        0.4 0.2 0.5 0.2]';
% map Gouezo groups to reefmod categories
rcs = [ rcs(:,1)/3 rcs(:,1)/3 rcs(:,1)/3 ...
        rcs(:,5) ...
        rcs(:,2)+rcs(:,4)+rcs(:,6) ...
        rcs(:,3)];
rcs = sum(rcs,1); % average across four sites
rcs = rcs./sum(rcs,2); % turn in relative covers
if META.doing_1AcroporaOnly == 1
   rcs = 1; 
end
    

% or give equal cover each
% rcs = ones(1,META.nb_coral_types)/META.nb_coral_types;

X = repmat(30, length(META.reef_ID),1); %30 cover across all reefs
Y = repmat(rcs,length(META.reef_ID),1);
...
if META.doing_genetics == 0 % NO ADAPTATION, NO THERMAL OPTIMUM
    ...
    if META.doing_RR_test
        % Make intense heatwave occur in summer of 16th year of DHW = 16
        % C-weeks to kill almost all corals
        DHW(:,15*2+1) = META.RR_test_DHWpert;
    elseif META.doing_TTacclim == 0 % ie regular DHWs without 0.1C/decade shift in thermal tolerance
        ...
        load('PAL_past_DHW_CRW_5km_1985_2020.mat') %
        PAL_PAST_DHW = PAL_PAST_DHW(:,24:36); %select 2008 to 2020
        
        end_hindcast = size(PAL_PAST_DHW,2);
        DHW(:,1:2:end_hindcast*2) = PAL_PAST_DHW(META.reef_ID,:);
        clear PAL_PAST_DHW
        ...
        if META.nb_time_steps>30 % now starts in 2023
            ...
            % load(['PAL_maxDHW_' char(OPTIONS.GCM) '_ssp' char(OPTIONS.SSP) '_2021_2100.mat'])
            load(['PAL_maxDHW_' char(OPTIONS.GCM) '_ssp' char(OPTIONS.SSP) '_2008_2100.mat'])
            ...
            if (META.nb_time_steps-30)/2 <= size(max_annual_DHW,2)
                ...
                DHW(:,(end_hindcast*2+1):2:end) = max_annual_DHW(META.reef_ID,12+(2:(META.nb_time_steps-24)/2));

                % 5-year transition (2016-2020, both) from satellite (<=2015) to GCM (>=2021)
                % 2016,17,18,19,20 is DHW index of 17,19,21,23,25, GCM index of 9,10,11,12,13
                if META.SatGCM_transition
                    % figure; plot(1:2:184,DHW(1,1:2:META.nb_time_steps)); hold on
                    DHW(:,17) = (5/6) * DHW(:,17) + (1/6) * max_annual_DHW(META.reef_ID,9);
                    DHW(:,19) = (4/6) * DHW(:,19) + (2/6) * max_annual_DHW(META.reef_ID,10);
                    DHW(:,21) = (3/6) * DHW(:,21) + (3/6) * max_annual_DHW(META.reef_ID,11);
                    DHW(:,23) = (2/6) * DHW(:,23) + (4/6) * max_annual_DHW(META.reef_ID,12);
                    DHW(:,25) = (1/6) * DHW(:,25) + (5/6) * max_annual_DHW(META.reef_ID,13);
                end
            else
        ...
        if META.doing_PPacclim
            % what is the proportion of reefs that could have some
            % acclimatisation in the form of pre-conditioning from
            % pre-pulses? Based on Ainsworth et al. 2016. Drop in mortality
            % probability for a pre-pulse is 45%
            % regression of warming on frequency of occurrence of
            % protective trajectories has intercept of 73, slope of -25.714
            % Warming = DHW;
            % Warming(:, 1:2:META.nb_time_steps) = movmean(DHW(:, 1:2:META.nb_time_steps), [5 5], 2)/12;
            % AcclimFreq = 73 + (-25.714) * Warming;
            AcclimFreq = zeros(length(META.reef_ID),META.nb_time_steps);
            AcclimFreq(:, 1:2:META.nb_time_steps) = 73 + (-25.714) * movmean(DHW(:, 1:2:META.nb_time_steps), [5 5], 2)/12;
        end
    else % ie acclimatisation DHWs with 0.1C/decade shift in thermal tolerance (DHW accumulation temp MMM+1)
        %% PAST THERMAL STRESS REGIME 2008-2020 with acclimatisation
        % Using Coral Reef Watch 5km product: max DHW every year from 1985 to 2020 from closest 5x5 km pixel
        load('PAL_past_DHW_0.1CpDec_CRW_5km_1985_2020.mat') %
        PAL_PAST_DHW = PAL_PAST_DHW(:,24:36); %select 2008 to 2020
        ...
        %% FUTURE THERMAL STRESS with acclimatisation
        %     if META.nb_time_steps>26
        if META.nb_time_steps>30 % now starts in 2023
            
            % Load the selected forecast scenario of DHW
            load(['PAL_maxDHW_0.1CpDec_' char(OPTIONS.GCM) '_ssp' char(OPTIONS.SSP) '_2021_2100.mat'])
