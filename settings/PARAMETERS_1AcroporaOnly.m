...
META.grid_x_count = 20; % number of grid cells along the x-edge was 20
META.grid_y_count = 20; % number of grid cells along the y-edge was 20
...
% REEF.initial_coral_cover = [ 0.05 ; 0.05; 0.05 ; 0.05 ; 0.05 ; 0.05];  % 3% total cover - meant to be 30%
REEF.initial_coral_cover = 3 ;  % 30% total cover
% REEF.initial_coral_cover = 3 ;  % 30% total cover

% Horizontal growth rate (radius extension) in cm per 6 month
% CORAL.growth_rate = [ 2.2 ; 2.2 ; 1.5 ; 1.2 ; 0.4 ; 0.6]; % NEW (2018)
% CORAL.growth_rate = mean([ 2.2 ; 2.2 ; 1.5]); % for mean Acropora
CORAL.growth_rate = 1.5; % for corymbose Acropora
...
% CORAL.SI = mean([ 6.16 ; 5.43 ; 5.65 ]) ; % mean Acropora
CORAL.SI = 5.65 ; % Corymbose acropora
...
CORAL.prop_settlers = 1; % one coral group
...
% CORAL.max_diameter = mean([120 ; 100 ; 50]); %  average acropora
CORAL.max_diameter = 50; %  Corymbose acropora
...
% CORAL.size_mean = mean([log(700) ; log(700) ; log(500) ]); % Average Acropora
CORAL.size_mean = log(500); % Corymbose Acropora
...
CORAL.size_var = log(2^2);  % Acropora
...
CORAL.lobophora_reduce_rate = 0.75/7; % Ortiz et al 2014
...
CORAL.dictyota_reduce_rate = 1.5 ; % for spawners only
...
% CORAL.fecund_min_size = mean([123; 123 ; 134]); % mean Acropora
CORAL.fecund_min_size = 134; % Corymbose Acropora
...
% CORAL.fecund_a = mean([1.03 ; 1.03 ; 1.69]); % mean Acropora
% CORAL.fecund_b = mean([1.28 ; 1.28 ; 1.05]); % mean Acropora
CORAL.fecund_a = 1.69 ; % Corymbose Acropora
CORAL.fecund_b = 1.05 ; % Corymbose Acropora
...
% CORAL.sensitivity_whole_natural = mean([0.1 ; 5 ; 4.5]) ; % mean Acropora - to adjust CORAL.adult_whole_mortality_rate to Pacific corals (Ortiz et al 2014)
% CORAL.sensitivity_partial_natural = 1; % mean Acropora - placeholder for when parmameters are available
% CORAL.extent_partial_natural = mean([2 ; 0.4 ; 1.4]) ; % mean Acropora - multiplicator of the extent of area lost (Ortiz et al 2014)
CORAL.sensitivity_whole_natural = 4.5 ; % Corymbose Acropora - to adjust CORAL.adult_whole_mortality_rate to Pacific corals (Ortiz et al 2014)
CORAL.sensitivity_partial_natural = 1 ; % Corymbose Acropora - placeholder for when parmameters are available
CORAL.extent_partial_natural = 1.4 ; % Corymbose Acropora - multiplicator of the extent of area lost (Ortiz et al 2014)
...
% CORAL.sensitivity_hurricane = C_adjust_GBR*mean([1.3 ; 1.3 ; 1.5]); % mean Acropora
CORAL.sensitivity_hurricane = C_adjust_GBR*1.5 ; % corymbose Acropora
...
% CORAL.sensitivity_bleaching = mean([1.5 ; 1.6 ; 1.4]);  % mean Acropora
CORAL.sensitivity_bleaching = 1.4 ;  % corymbose Acropora
...
CORAL.bleaching_partial_extent = 0.05 ; % Acropora
...
CORAL.proba_switching = 0.5 ; % Acropora
...
% Bleaching mortality from Lachs et al 2022 experiment
% logit(BM) = B0 + B1 * DHW, 
% where B1 = 0.87, and the B0 values (from curves fit to all colonies in
% the experiment) range = -2 to -10.5, mean = -6.86, SD = 1.02
% However, we can use a more intuitive metric to think of heat tolerance
% the DHW value at which mortality probability passes 0.5 (DHWmort50)
% correlates 100% with the intercept and is:
% DHW = (logit(0.5) - B0)/ B1, but logit(0.5) = 0, so
% DHWmort50 = -B0/B1
% then the mean and SD of DHWmort50 values across all colonies are:
% mean = 7.887, SD = 1.177
% CORAL.DHWbleaching_mortality_h2 = 0.4;
% note that with shifting baseline, the analysis of 2022 experiment yields
% different parameters for coral heat tolerance
if OPTIONS.doing_TTacclim==0 % normal without 0.1C/decade increase in tolerance
    CORAL.DHWbleaching_mortality_Slope = 0.74;
    CORAL.DHWbleaching_mortality_Intercept_av = -6.24;
    CORAL.DHWbleaching_mortality_Intercept_sd = 0.98;
else % with increase of 0.1C/decade in DHW computation
    CORAL.DHWbleaching_mortality_Slope = 0.84;
    CORAL.DHWbleaching_mortality_Intercept_av = -6.06;
    CORAL.DHWbleaching_mortality_Intercept_sd = 0.98;
end
CORAL.DHWbleaching_mortality_Intercept_super_tolerant = -10.16; % the upper limit of heat tolerance seen to date
% CORAL.DHWbleaching_mortality_Slope = 0.87;
% CORAL.DHWbleaching_mortality_Intercept_av = -6.86;
% CORAL.DHWbleaching_mortality_Intercept_sd = 1.01;
% CORAL.DHWbleaching_mortality_Intercept_super_tolerant = -10.48981; % the upper limit of heat tolerance seen to date
% CORAL.DHWbleaching_mortality_DHWmort50_av = 7.887;
% CORAL.DHWbleaching_mortality_DHWmort50_av = 1.177;
% CORAL.DHWbleaching_mortality_Intercept_sd = 0;
% CORAL.DHWsensitivity_bleaching = [0.93 ; 0.88 ; 1 ; 0.82 ; 1.35 ; 1.35]; % GBR after calibration with Hughes et al (2018)
% CORAL.DHWsensitivity_bleaching = mean([0.93 ; 0.88 ; 1]); % mean Acropora
CORAL.DHWsensitivity_bleaching = 1; % Corymbose Acropora
% A multiplicator of the HT intercepts to get bleaching for a DHW 10 event
% to go to around 35%, compared to A. digitifera with ~85%, also to match
% the previous reefmod implementation.
